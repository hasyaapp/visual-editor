...
`)}function ed(r){let a=String(r||"").trim(),s=a.match(/^#([0-9a-f]{3}|[0-9a-f]{6})$/i);if(s){let l=s[1];l.length===3&&(l=l.split("").map(d=>d+d).join(""));let h=parseInt(l,16);return[h>>16&255,h>>8&255,h&255].join(", ")}let o=a.match(/^rgba?\(\s*(\d{1,3})\s*[,\s]\s*(\d{1,3})\s*[,\s]\s*(\d{1,3})/i);return o?[o[1],o[2],o[3]].join(", "):""}function $l(r,a,s){let o=a.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),l=new RegExp("("+o+"\\s*:\\s*)([^;{}]+)(;)","g");return String(r||"").replace(l,"$1"+s+"$3")}function Pl(r,a){let s=o=>{if(o)try{o.documentElement?.style?.setProperty(r,a),o.body?.style?.setProperty(r,a),o.querySelector("[data-sve-template]")?.style?.setProperty(r,a)}catch{}};C("iframe").forEach(o=>{try{s(o.contentDocument)}catch{}})}function Re(r,a){let s=["css","head","html"],o=null;for(let S of s)if(hi(D(S),r)){o=S;break}if(!o)return!1;let l=D(o),h=$l(l,r,a),d=r+"-rgb",y=ed(a);return y&&hi(l,d)&&(h=$l(h,d,y)),h===l?!1:(ot(o,h),Pl(r,a),y&&hi(l,d)&&Pl(d,y),He(),!0)}function Qi(r){return String(u.defaults?.cssTokens?.[r]||"").trim()}function oe(r){let a=String(r?.type||"text").trim().toLowerCase();return a==="datetime-local"?"datetime":a==="checkbox"?"boolean":a}function td(r,a){return r?.readOnly===!0||r?.readonly===!0||r?.locked===!0||Yr(r,a)}function Nl(r){if(r&&Object.prototype.hasOwnProperty.call(r,"default"))return gt(r.default);let a=oe(r);return a==="boolean"?!1:""}function id(r){return(Array.isArray(r?.options)?r.options:[]).map(s=>{if(s&&typeof s=="object"&&!Array.isArray(s)){let o=s.value??s.id??s.key??"";return{value:String(o),label:String(s.label??s.name??o)}}return{value:String(s??""),label:String(s??"")}})}function rd(r){let a=[];return["min","max","step","maxlength","minlength","pattern"].forEach(s=>{r?.[s]!==void 0&&r?.[s]!==null&&String(r[s])!==""&&a.push(`${s}="${A(r[s])}"`)}),r?.placeholder&&a.push(`placeholder="${A(r.placeholder)}"`),a.join(" ")}function nd(r){let a=String(r?.help||r?.description||"").trim();return a?`
      ...
      `:""}function ad(r,a){let s=V(u.config,a),o=oe(r),l=Yr(r,a),h=td(r,a),d=l?Et()||s||"":s??"",y=`data-field-path="${A(a)}" data-field-type="${A(o)}" aria-label="${A(r?.label||a)}" `+(h?'data-field-readonly="1" ':""),S=rd(r);if(o==="textarea")return`
        <textarea
...
`).trim();return a!==r.trim()?(ot("css",a),!0):!1}function bd(r){if(!r||!u.config)return;Array.from(r.querySelectorAll('[data-sve-type="image"][data-sve-field]')).forEach(s=>{let o=s.getAttribute("data-sve-field");if(!o)return;let l=Ut(o),h=s.closest("[data-sve-image-wrapper]"),d=h||s,y=l.align==="left"?"0":"auto",S=l.align==="right"?"0":"auto";h?(h.style.display=l.hidden?"none":"",h.style.width=l.width+"%",h.style.maxWidth="100%",h.style.marginLeft=y,h.style.marginRight=S,s.style.width="100%"):(s.style.display=l.hidden?"none":"",s.style.width=l.width+"%",s.style.maxWidth="100%",s.style.marginLeft=y,s.style.marginRight=S),l.fit==="auto"?s.style.removeProperty("object-fit"):s.style.objectFit=l.fit;let w=Xp[l.alignPos]||"";w?s.style.objectPosition=w:s.style.removeProperty("object-position"),s.style.height="100%"})}function kx(){pi.request({images:!0})}function xd(r){let a={"top left":`
    ...
    `}function Ld(){return u.config?Td()?["Background","Body Teks","Button Primary","Button Secondary"].map(a=>`
            <div class="group">
              <div class="group-title">
                ${a}
              </div>
