...
`).trim();return a!==r.trim()?(ot("css",a),!0):!1}function fd(r){if(!r||!u.config)return;Array.from(r.querySelectorAll('[data-sve-type="image"][data-sve-field]')).forEach(s=>{let o=s.getAttribute("data-sve-field");if(!o)return;let c=Ut(o),h=s.closest("[data-sve-image-wrapper]"),m=h||s,v=c.align==="left"?"0":"auto",S=c.align==="right"?"0":"auto";h?(h.style.display=c.hidden?"none":"",h.style.width=c.width+"%",h.style.maxWidth="100%",h.style.marginLeft=v,h.style.marginRight=S,s.style.width="100%"):(s.style.display=c.hidden?"none":"",s.style.width=c.width+"%",s.style.maxWidth="100%",s.style.marginLeft=v,s.style.marginRight=S),c.fit==="auto"?s.style.removeProperty("object-fit"):s.style.objectFit=c.fit;let w=Zp[c.alignPos]||"";w?s.style.objectPosition=w:s.style.removeProperty("object-position"),s.style.height="100%"})}function yx(){pi.request({images:!0})}function md(r){let a={"top left":`
    ...
    `}function gd(r){let a=Ut(r.path),s=c=>c==="left"?`
          <svg
            width="1em"
            height="1em"
            viewBox="0 0 21 20"
    ...
    `}function kd(r,a){let s=B(u.config,r);if(!Array.isArray(s)||a<0||a>=s.length)return;let o=pd(r),c=pn(o),h=String(c?.key||"src"),m=zl(),v={};for(let S=0;S<s.length;S++){let w=r+"."+S+"."+h;Object.prototype.hasOwnProperty.call(m,w)&&(v[S]=gt(m[w]))}s.splice(a,1),Object.keys(m).forEach(S=>{S.startsWith(r+".")&&S.endsWith("."+h)&&delete m[S]});for(let S=0;S<s.length;S++){let w=S<a?S:S+1,A=v[w];A&&(m[r+"."+S+"."+h]=A)}hd(),Te("Foto gallery dihapus"),ce()}function Sd(r){u.config&&(He(u.config,r,""),Ht(r,{hidden:!0}),Te("Gambar dihapus"),ce())}function dn(r){let a=B(u.config,r.path)||"",s=Ut(r.path),c=`
    ...
    `}function Ad(){return u.config?Cd()?["Background","Body Teks","Button Primary","Button Secondary"].map(a=>`
            <div class="group">
              <div class="group-title">
                ${a}
              </div>

              ${_.filter(s=>s[0]===a).map(([,s,o])=>Ed(s,o)).join("")}
            </div>
          `).join("")+`
        <button
          type="button"
...
`,ot("head",m)}let h=document.getElementById(e+"-font-link");h||(h=document.createElement("link"),h.id=e+"-font-link",h.rel="stylesheet",document.head.appendChild(h)),h.href=r,Gl()}async function rr(r){let a=k("#"+e+"-"+r+"-font");if(!a)return;let s=_d(a.value);if(!s.valid||!s.family)return;let o=s.family;a.value=o;let c=await Ld(o,r);if(!c.ok){c.reason==="stylesheet"||c.reason==="font-file"||c.reason;return}let h=r==="heading"?"--sve-font-heading":"--sve-font-body",m=r==="heading"?"--sve-heading-weight":"--sve-body-weight";c.normalizedWeight&&c.weight&&Re(m,c.weight),He(u.config,"editorStyle.googleFonts."+r,o),Te(),Re(h,`"${o}", ${$d(o,r)}`),Fd(r,!1),ir(),Gl(),ze()}function nr(r,a,s=!0,o=!1){let c=String(r||"").trim(),h=s&&c&&!a.includes(c)?[c,...a]:[...a];return o&&(h=[...new Set(h)].sort((m,v)=>{let S=Number.parseFloat(m),w=Number.parseFloat(v);return Number.isFinite(S)&&Number.isFinite(w)?S-w:String(m).localeCompare(String(v))})),h.map((m,v)=>{let S=a.includes(c)||s?m===c:v===0;return`
          ...
          `}).join("")}function Md(r){let a=be(r.variable)||r.fallback;if(r.type==="size")return`
        <select
          class="style-select"
      ...
      `;if(r.type==="weight"){let s=Pd(r.variable),o=s?Rd(s):Y,c=s?tr(s,a):a;return`
        <select
          class="style-select"
          data-style-var="${E(r.variable)}"
        >
...
`);/\beval\s*\(/.test(L)&&c("eval() terdeteksi"),/\bnew\s+Function\s*\(/.test(L)&&c("new Function() terdeteksi"),/javascript\s*:/i.test(L)&&c("javascript: URL terdeteksi"),/https?:\/\/[^\s"']*scalev\.(?:com|id)\/api\//i.test(L)&&c("Private Scalev API URL terdeteksi"),/(service[_-]?role|database[_-]?password|private[_-]?api[_-]?key|secret[_-]?token)\s*[:=]/i.test(L)&&c("Kemungkinan secret/private credential terdeteksi"),ec(c,h,m);let U=D("js");/\bconst\s+CONFIG\s*=/.test(U)||s.push("CONFIG strict canonical sebaiknya memakai const"),/\bconst\s+SVE_SCHEMA\s*=/.test(U)||s.push("SVE_SCHEMA strict canonical sebaiknya memakai const");let ne=B(u.config,"sections.rsvp")===!0,At=B(u.config,"sections.guestbook")===!0,X=String(B(u.config,"rsvp.endpoint")||""),We=B(u.config,"rsvp.enabled"),_e=!!X||We!==void 0;if(ne)if(_e)We!==!0&&c("RSVP & Ucapan visible tetapi rsvp.enabled bukan true"),/^https:\/\//i.test(X)||c("RSVP & Ucapan membutuhkan endpoint HTTPS");else{let F=String(B(u.config,"extensions.rsvpBackend.mode")||"none");if(F!=="none"&&F!=="external"&&c("RSVP backend mode harus none atau external"),F==="external"){let Ie=String(B(u.config,"extensions.rsvpBackend.endpoint")||"");/^https:\/\//i.test(Ie)||c("RSVP external membutuhkan endpoint HTTPS")}else s.push("RSVP backend belum dikonfigurasi; public runtime wajib fail-closed")}if(At){let F=B(u.config,"guestbook.enabled"),Ie=String(B(u.config,"guestbook.endpoint")||"");F!==!0&&c("Ucapan & Doa legacy visible tetapi guestbook.enabled bukan true"),/^https:\/\//i.test(Ie)||c("Ucapan & Doa legacy visible tetapi endpoint HTTPS belum valid")}let bt=on(),yi=oe.map(F=>F.variable).filter(F=>!hi(bt,F));yi.length?c("Typography role tokens belum lengkap: "+yi.join(", ")):m("Semua typography role tokens tersedia"),/(?:\.svw-(?:cover-names|heading|quote-text|person-name|item-title|date-display|count\s+strong|gallery-caption|event-meta|field\s+label|footer-brand|footer-creator|footer-note|btn|kicker))[^\{]*\{[^\}]*font-size\s*:\s*(?!var\()/is.test(bt)&&h("Terdeteksi typography editorial hardcoded; map seluruh teks ke role token --sve-*.");let lc=Xl();return Object.values(lc).reduce((F,Ie)=>F+Ie.length,0)?s.push("External origin terdeteksi; salin CSP manifest ke Scalev Security"):m("Tidak ada external origin wajib dari scanner"),{status:a.length?"BLOCKER":s.length?"WARNING":"PASS",blockers:a,warnings:s,passes:o,csp:lc}}function Hd(r){return r==="PASS"?`
    ...
    `}function zd(){if(!u.config)return fi();let r=ic(),a=(c,h)=>c.length?`<ul>${c.map(m=>`<li>${E(m)}</li>`).join("")}</ul>`:`<p class="compat-empty">${E(h)}</p>`,s=r.status==="PASS"?"Siap":r.status==="WARNING"?"Perlu dicek":"Masalah",o=r.status==="PASS"?"Semua siap":r.status==="WARNING"?"Perlu diperiksa":"Perlu diperbaiki";return`
    ...
    `}function Wd(r){let a=[],s=new WeakSet,o=(c,h="CONFIG")=>{if(c!==null){if(typeof c=="object"){if(s.has(c)){a.push("Referensi berulang: "+h);return}s.add(c)}if(Array.isArray(c)){c.forEach((m,v)=>o(m,h+"."+v));return}if(typeof c=="object"){Object.keys(c).forEach(m=>{z.has(m)&&a.push("Forbidden key: "+h+"."+m),o(c[m],h+"."+m)});return}["string","number","boolean"].includes(typeof c)||a.push("Non-static value: "+h),typeof c=="number"&&!Number.isFinite(c)&&a.push("Non-finite number: "+h)}};o(r);try{JSON.parse(JSON.stringify(r))}catch{a.push("CONFIG gagal round-trip JSON")}return a}function Gd(){let r=u.templateLibrary,a=String(u.search||"").trim().toLowerCase(),s=r.templates.filter(h=>a?[h.name].join(" ").toLowerCase().includes(a):!0);r.status==="idle"&&gl().then(()=>{u.tab==="library"&&(u.uiPrepared=!1,ce())});let o=r.error?`
