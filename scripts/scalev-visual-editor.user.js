...
PIN lama langsung tidak berlaku. Kalau sudah dikirim ke klien, PIN baru ini harus dikirim ulang.`))){a.busy=!0,a.status="loading",a.message="",nt();try{let h=await(await Qr(Nh,{method:"POST",headers:{"Content-Type":"application/json","x-team-key":o},body:JSON.stringify({weddingId:s,mode:r==="generate"?"generate":"peek"})})).json();!h||h.ok!==!0?(a.status="error",a.pin="",a.message=Dh[h&&h.error]||"Gagal mengambil PIN."):(a.status="ready",a.slug=s,a.pin=String(h.pin||""),a.version=Number(h.version)||0,a.message=h.regenerated?"PIN baru dibuat. Kirim ulang ke klien.":"")}catch{a.status="error",a.pin="",a.message="Tidak bisa menghubungi server."}a.busy=!1,nt()}}function Vh(){let r=v("#"+e+"-team-key"),a=r?r.value.trim():"",s=u.dashboardPin;if(!a){s.message="Kunci tim belum diisi.",nt();return}if(!Oh(a)){s.message="Tampermonkey menolak menyimpan kunci.",nt();return}s.status="idle",s.message="",tn("peek")}function Bh(){let r=u.dashboardPin;if(!Mh()){r.message="Tampermonkey menolak menghapus kunci.",nt();return}r.status="needkey",r.pin="",r.version=0,r.message="",nt()}async function jh(){let r=u.dashboardPin;if(r.pin){try{await navigator.clipboard.writeText(r.pin),r.message="PIN tersalin."}catch{r.message="Gagal menyalin. Salin manual dari kolom PIN."}nt()}}function nt(){let r=document.activeElement?.id,a=v("#"+e+"-pin-panel");a&&(a.innerHTML=Sl());let s=v("#"+e+"-pin-pill");s&&(s.outerHTML=Nl()),r?.startsWith(e+"-pin-")&&document.getElementById(r)?.focus({preventScroll:!0})}function Sl(){let r=u.dashboardPin,a=en(),s=m=>m?`<small class="auto-wedding-id-note">${E(m)}</small>`:"";if(!a)return`
    ...
    `}function wl(){if(!u.defaults)return;u.defaults.config&&(u.config=Pe(u.defaults.config),xe()),Object.entries(u.defaults.cssTokens||{}).forEach(([a,s])=>{s&&je(a,s)});let r=u.defaults.googleFonts||{};u.config&&V(u.config,"editorStyle.googleFonts")!==void 0&&(de(u.config,"editorStyle.googleFonts",Pe(r)),xe(),Qi()),Te(),ne(),Be()}function Uh({syncImages:r=!1,delay:a=100}={}){Be({syncImages:r})}let Ui=Bp({document,window,getConfig:()=>u.config,syncImages:ud,metrics:u.performance});function Be({syncImages:r=!1,force:a=!1}={}){Ui.request({images:r,force:a})}function Hh(r){if(!r)return"";let a=new Date(r);if(Number.isNaN(a.getTime()))return"";let s=o=>String(o).padStart(2,"0");return a.getFullYear()+"-"+s(a.getMonth()+1)+"-"+s(a.getDate())+"T"+s(a.getHours())+":"+s(a.getMinutes())}function zh(r){if(!r)return"";let a=new Date(r),s=h=>String(h).padStart(2,"0"),o=-a.getTimezoneOffset(),c=o>=0?"+":"-";return r+":00"+c+s(Math.floor(Math.abs(o)/60))+":"+s(Math.abs(o)%60)}function ye(r,a){let s=a?[a]:[nn()],o=r.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),c=new RegExp(o+"\\s*:\\s*([^;{}]+);");for(let h of s){let m=c.exec(h||"");if(m)return m[1].trim()}return""}function rn(r,a){let s=a.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");return new RegExp(s+"\\s*:\\s*[^;{}]+;").test(r||"")}function nn(){let r=[],a=D("css");return a&&r.push(a),[D("html"),D("head")].forEach(s=>{let o=String(s||""),c=/<style\b[^>]*>([\s\S]*?)<\/style>/gi,h;for(;h=c.exec(o);)h[1]&&r.push(h[1])}),r.join(`
`)}function Wh(r,a,s){let o=a.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),c=new RegExp("("+o+"\\s*:\\s*)([^;{}]+)(;)","g");return String(r||"").replace(c,"$1"+s+"$3")}function Gh(r,a){let s=o=>{if(o)try{o.documentElement?.style?.setProperty(r,a),o.body?.style?.setProperty(r,a),o.querySelector("[data-sve-template]")?.style?.setProperty(r,a)}catch{}};C("iframe").forEach(o=>{try{s(o.contentDocument)}catch{}})}function je(r,a){let s=["css","head","html"],o=null;for(let m of s)if(rn(D(m),r)){o=m;break}if(!o)return!1;let c=D(o),h=Wh(c,r,a);return h===c?!1:(rt(o,h),Gh(r,a),Be(),!0)}function Hi(r){return String(u.defaults?.cssTokens?.[r]||"").trim()}function ve(r){let a=String(r?.type||"text").trim().toLowerCase();return a==="datetime-local"?"datetime":a==="checkbox"?"boolean":a}function qh(r,a){return r?.readOnly===!0||r?.readonly===!0||r?.locked===!0||zr(r,a)}function Cl(r){if(r&&Object.prototype.hasOwnProperty.call(r,"default"))return Pe(r.default);let a=ve(r);return a==="boolean"?!1:""}function Kh(r){return(Array.isArray(r?.options)?r.options:[]).map(s=>{if(s&&typeof s=="object"&&!Array.isArray(s)){let o=s.value??s.id??s.key??"";return{value:String(o),label:String(s.label??s.name??o)}}return{value:String(s??""),label:String(s??"")}})}function Yh(r){let a=[];return["min","max","step","maxlength","minlength","pattern"].forEach(s=>{r?.[s]!==void 0&&r?.[s]!==null&&String(r[s])!==""&&a.push(`${s}="${E(r[s])}"`)}),r?.placeholder&&a.push(`placeholder="${E(r.placeholder)}"`),a.join(" ")}function Qh(r){let a=String(r?.help||r?.description||"").trim();return a?`
        <small class="field-help">
          ${E(a)}
        </small>
      `:""}function Zh(r,a){let s=V(u.config,a),o=ve(r),c=zr(r,a),h=qh(r,a),m=c?vt()||s||"":s??"",k=`data-field-path="${E(a)}" data-field-type="${E(o)}" aria-label="${E(r?.label||a)}" `+(h?'data-field-readonly="1" ':""),S=Yh(r);if(o==="textarea")return`
        <textarea
...
`).trim();return a!==r.trim()?(rt("css",a),!0):!1}function ud(r){if(!r||!u.config)return;Array.from(r.querySelectorAll('[data-sve-type="image"][data-sve-field]')).forEach(s=>{let o=s.getAttribute("data-sve-field");if(!o)return;let c=Dt(o),h=s.closest("[data-sve-image-wrapper]"),m=h||s,k=c.align==="left"?"0":"auto",S=c.align==="right"?"0":"auto";h?(h.style.display=c.hidden?"none":"",h.style.width=c.width+"%",h.style.maxWidth="100%",h.style.marginLeft=k,h.style.marginRight=S,s.style.width="100%"):(s.style.display=c.hidden?"none":"",s.style.width=c.width+"%",s.style.maxWidth="100%",s.style.marginLeft=k,s.style.marginRight=S),c.fit==="auto"?s.style.removeProperty("object-fit"):s.style.objectFit=c.fit;let w=qp[c.alignPos]||"";w?s.style.objectPosition=w:s.style.removeProperty("object-position"),s.style.height="100%"})}function hx(){Ui.request({images:!0})}function pd(r){let a={"top left":`
    ...
    `}function hd(r){let a=Dt(r.path),s=c=>c==="left"?`
          <svg
            width="1em"
            height="1em"
            viewBox="0 0 21 20"
    ...
    `}function bd(r,a){let s=V(u.config,r);if(!Array.isArray(s)||a<0||a>=s.length)return;let o=od(r),c=on(o),h=String(c?.key||"src"),m=Ml(),k={};for(let S=0;S<s.length;S++){let w=r+"."+S+"."+h;Object.prototype.hasOwnProperty.call(m,w)&&(k[S]=Pe(m[w]))}s.splice(a,1),Object.keys(m).forEach(S=>{S.startsWith(r+".")&&S.endsWith("."+h)&&delete m[S]});for(let S=0;S<s.length;S++){let w=S<a?S:S+1,A=k[w];A&&(m[r+"."+S+"."+h]=A)}ld(),xe("Foto gallery dihapus"),ne()}function xd(r){u.config&&(de(u.config,r,""),Vt(r,{hidden:!0}),xe("Gambar dihapus"),ne())}function yd(){if(!u.config||!u.defaultConfig)return;let r=sd();r&&de(u.config,r.path,Pe(V(u.defaultConfig,r.path)||[])),qi().filter(a=>!a.gallery).forEach(a=>{de(u.config,a.path,Pe(V(u.defaultConfig,a.path))??"")}),u.config.imageSettings=Pe(u.defaultConfig.imageSettings||{}),xe(),ne()}function vd(){return u.config?qi().filter(s=>u.search?(s.label+" "+s.path).toLowerCase().includes(u.search):!0).map(s=>{let o=V(u.config,s.path)||"",c=Dt(s.path),m=`
    ...
    `}function Cd(){return u.config?Sd()?["Background","Body Teks","Button Primary","Button Secondary","Aksen"].map(a=>`
            <div class="group">
              <div class="group-title">
                ${a}
              </div>

              ${I.filter(s=>s[0]===a).map(([,s,o])=>wd(s,o)).join("")}
            </div>
          `).join("")+`
        <button
          type="button"
...
`,rt("head",m)}let h=document.getElementById(e+"-font-link");h||(h=document.createElement("link"),h.id=e+"-font-link",h.rel="stylesheet",document.head.appendChild(h)),h.href=r,Vl()}async function Zi(r){let a=v("#"+e+"-"+r+"-font");if(!a)return;let s=Ad(a.value);if(!s.valid||!s.family)return;let o=s.family;a.value=o;let c=await _d(o,r);if(!c.ok){c.reason==="stylesheet"||c.reason==="font-file"||c.reason;return}let h=r==="heading"?"--sve-font-heading":"--sve-font-body",m=r==="heading"?"--sve-heading-weight":"--sve-body-weight";c.normalizedWeight&&c.weight&&je(m,c.weight),de(u.config,"editorStyle.googleFonts."+r,o),xe(),je(h,`"${o}", ${Id(o,r)}`),Nd(r,!1),Qi(),Vl(),Be()}function Ji(r,a,s=!0,o=!1){let c=String(r||"").trim(),h=s&&c&&!a.includes(c)?[c,...a]:[...a];return o&&(h=[...new Set(h)].sort((m,k)=>{let S=Number.parseFloat(m),w=Number.parseFloat(k);return Number.isFinite(S)&&Number.isFinite(w)?S-w:String(m).localeCompare(String(k))})),h.map((m,k)=>{let S=a.includes(c)||s?m===c:k===0;return`
          ...
          `}).join("")}function Rd(r){let a=ye(r.variable)||r.fallback;if(r.type==="size")return`
        <select
          class="style-select"
      ...
      `;if(r.type==="weight"){let s=Ld(r.variable),o=s?Pd(s):K,c=s?Yi(s,a):a;return`
        <select
          class="style-select"
          data-style-var="${E(r.variable)}"
        >
...
`);/\beval\s*\(/.test(P)&&c("eval() terdeteksi"),/\bnew\s+Function\s*\(/.test(P)&&c("new Function() terdeteksi"),/javascript\s*:/i.test(P)&&c("javascript: URL terdeteksi"),/https?:\/\/[^\s"']*scalev\.(?:com|id)\/api\//i.test(P)&&c("Private Scalev API URL terdeteksi"),/(service[_-]?role|database[_-]?password|private[_-]?api[_-]?key|secret[_-]?token)\s*[:=]/i.test(P)&&c("Kemungkinan secret/private credential terdeteksi"),Gl(c,h,m);let G=D("js");/\bconst\s+CONFIG\s*=/.test(G)||s.push("CONFIG strict canonical sebaiknya memakai const"),/\bconst\s+SVE_SCHEMA\s*=/.test(G)||s.push("SVE_SCHEMA strict canonical sebaiknya memakai const");let oe=V(u.config,"sections.rsvp")===!0,kt=V(u.config,"sections.guestbook")===!0,J=String(V(u.config,"rsvp.endpoint")||""),Ue=V(u.config,"rsvp.enabled"),_e=!!J||Ue!==void 0;if(oe)if(_e)Ue!==!0&&c("RSVP & Ucapan visible tetapi rsvp.enabled bukan true"),/^https:\/\//i.test(J)||c("RSVP & Ucapan membutuhkan endpoint HTTPS");else{let F=String(V(u.config,"extensions.rsvpBackend.mode")||"none");if(F!=="none"&&F!=="external"&&c("RSVP backend mode harus none atau external"),F==="external"){let Ie=String(V(u.config,"extensions.rsvpBackend.endpoint")||"");/^https:\/\//i.test(Ie)||c("RSVP external membutuhkan endpoint HTTPS")}else s.push("RSVP backend belum dikonfigurasi; public runtime wajib fail-closed")}if(kt){let F=V(u.config,"guestbook.enabled"),Ie=String(V(u.config,"guestbook.endpoint")||"");F!==!0&&c("Ucapan & Doa legacy visible tetapi guestbook.enabled bukan true"),/^https:\/\//i.test(Ie)||c("Ucapan & Doa legacy visible tetapi endpoint HTTPS belum valid")}let pt=nn(),ci=se.map(F=>F.variable).filter(F=>!rn(pt,F));ci.length?c("Typography role tokens belum lengkap: "+ci.join(", ")):m("Semua typography role tokens tersedia"),/(?:\.svw-(?:cover-names|heading|quote-text|person-name|item-title|date-display|count\s+strong|gallery-caption|event-meta|field\s+label|footer-brand|footer-creator|footer-note|btn|kicker))[^\{]*\{[^\}]*font-size\s*:\s*(?!var\()/is.test(pt)&&h("Terdeteksi typography editorial hardcoded; map seluruh teks ke role token --sve-*.");let tc=Wl();return Object.values(tc).reduce((F,Ie)=>F+Ie.length,0)?s.push("External origin terdeteksi; salin CSP manifest ke Scalev Security"):m("Tidak ada external origin wajib dari scanner"),{status:a.length?"BLOCKER":s.length?"WARNING":"PASS",blockers:a,warnings:s,passes:o,csp:tc}}function jd(r){return r==="PASS"?`
    ...
    `}function Ud(){if(!u.config)return Ot();let r=Kl(),a=(c,h)=>c.length?`<ul>${c.map(m=>`<li>${E(m)}</li>`).join("")}</ul>`:`<p class="compat-empty">${E(h)}</p>`,s=r.status==="PASS"?"Siap":r.status==="WARNING"?"Perlu dicek":"Masalah",o=r.status==="PASS"?"Semua siap":r.status==="WARNING"?"Perlu diperiksa":"Perlu diperbaiki";return`
    ...
    `}function Hd(r){let a=[],s=new WeakSet,o=(c,h="CONFIG")=>{if(c!==null){if(typeof c=="object"){if(s.has(c)){a.push("Referensi berulang: "+h);return}s.add(c)}if(Array.isArray(c)){c.forEach((m,k)=>o(m,h+"."+k));return}if(typeof c=="object"){Object.keys(c).forEach(m=>{Br.has(m)&&a.push("Forbidden key: "+h+"."+m),o(c[m],h+"."+m)});return}["string","number","boolean"].includes(typeof c)||a.push("Non-static value: "+h),typeof c=="number"&&!Number.isFinite(c)&&a.push("Non-finite number: "+h)}};o(r);try{JSON.parse(JSON.stringify(r))}catch{a.push("CONFIG gagal round-trip JSON")}return a}function zd(){let r=u.templateLibrary,a=String(u.search||"").trim().toLowerCase(),s=r.templates.filter(h=>a?[h.name].join(" ").toLowerCase().includes(a):!0);r.status==="idle"&&ul().then(()=>{u.tab==="library"&&(u.uiPrepared=!1,ne())});let o=r.error?`
