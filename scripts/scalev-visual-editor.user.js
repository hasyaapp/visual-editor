...
`);/\beval\s*\(/.test(P)&&c("eval() terdeteksi"),/\bnew\s+Function\s*\(/.test(P)&&c("new Function() terdeteksi"),/javascript\s*:/i.test(P)&&c("javascript: URL terdeteksi"),/https?:\/\/[^\s"']*scalev\.(?:com|id)\/api\//i.test(P)&&c("Private Scalev API URL terdeteksi"),/(service[_-]?role|database[_-]?password|private[_-]?api[_-]?key|secret[_-]?token)\s*[:=]/i.test(P)&&c("Kemungkinan secret/private credential terdeteksi"),Gl(c,h,m);let G=D("js");/\bconst\s+CONFIG\s*=/.test(G)||s.push("CONFIG strict canonical sebaiknya memakai const"),/\bconst\s+SVE_SCHEMA\s*=/.test(G)||s.push("SVE_SCHEMA strict canonical sebaiknya memakai const");let oe=V(u.config,"sections.rsvp")===!0,kt=V(u.config,"sections.guestbook")===!0,J=String(V(u.config,"rsvp.endpoint")||""),je=V(u.config,"rsvp.enabled"),_e=!!J||je!==void 0;if(oe)if(_e)je!==!0&&c("RSVP & Ucapan visible tetapi rsvp.enabled bukan true"),/^https:\/\//i.test(J)||c("RSVP & Ucapan membutuhkan endpoint HTTPS");else{let F=String(V(u.config,"extensions.rsvpBackend.mode")||"none");if(F!=="none"&&F!=="external"&&c("RSVP backend mode harus none atau external"),F==="external"){let Ie=String(V(u.config,"extensions.rsvpBackend.endpoint")||"");/^https:\/\//i.test(Ie)||c("RSVP external membutuhkan endpoint HTTPS")}else s.push("RSVP backend belum dikonfigurasi; public runtime wajib fail-closed")}if(kt){let F=V(u.config,"guestbook.enabled"),Ie=String(V(u.config,"guestbook.endpoint")||"");F!==!0&&c("Ucapan & Doa legacy visible tetapi guestbook.enabled bukan true"),/^https:\/\//i.test(Ie)||c("Ucapan & Doa legacy visible tetapi endpoint HTTPS belum valid")}let ut=rn(),ci=se.map(F=>F.variable).filter(F=>!tn(ut,F));ci.length?c("Typography role tokens belum lengkap: "+ci.join(", ")):m("Semua typography role tokens tersedia"),/(?:\.svw-(?:cover-names|heading|quote-text|person-name|item-title|date-display|count\s+strong|gallery-caption|event-meta|field\s+label|footer-brand|footer-creator|footer-note|btn|kicker))[^\{]*\{[^\}]*font-size\s*:\s*(?!var\()/is.test(ut)&&h("Terdeteksi typography editorial hardcoded; map seluruh teks ke role token --sve-*.");let tc=Wl();return Object.values(tc).reduce((F,Ie)=>F+Ie.length,0)?s.push("External origin terdeteksi; salin CSP manifest ke Scalev Security"):m("Tidak ada external origin wajib dari scanner"),{status:a.length?"BLOCKER":s.length?"WARNING":"PASS",blockers:a,warnings:s,passes:o,csp:tc}}function Vd(r){return r==="PASS"?`
    ...
    `}function Ud(r){let a=[],s=new WeakSet,o=(c,h="CONFIG")=>{if(c!==null){if(typeof c=="object"){if(s.has(c)){a.push("Referensi berulang: "+h);return}s.add(c)}if(Array.isArray(c)){c.forEach((m,k)=>o(m,h+"."+k));return}if(typeof c=="object"){Object.keys(c).forEach(m=>{Br.has(m)&&a.push("Forbidden key: "+h+"."+m),o(c[m],h+"."+m)});return}["string","number","boolean"].includes(typeof c)||a.push("Non-static value: "+h),typeof c=="number"&&!Number.isFinite(c)&&a.push("Non-finite number: "+h)}};o(r);try{JSON.parse(JSON.stringify(r))}catch{a.push("CONFIG gagal round-trip JSON")}return a}function jd(){let r=u.templateLibrary,a=String(u.search||"").trim().toLowerCase(),s=r.templates.filter(h=>a?[h.name].join(" ").toLowerCase().includes(a):!0);r.status==="idle"&&cl().then(()=>{u.tab==="library"&&(u.uiPrepared=!1,ne())});let o=r.error?`
