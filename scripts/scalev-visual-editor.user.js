...
`)}function ed(r){let a=String(r||"").trim(),s=a.match(/^#([0-9a-f]{3}|[0-9a-f]{6})$/i);if(s){let l=s[1];l.length===3&&(l=l.split("").map(d=>d+d).join(""));let h=parseInt(l,16);return[h>>16&255,h>>8&255,h&255].join(", ")}let o=a.match(/^rgba?\(\s*(\d{1,3})\s*[,\s]\s*(\d{1,3})\s*[,\s]\s*(\d{1,3})/i);return o?[o[1],o[2],o[3]].join(", "):""}function $l(r,a,s){let o=a.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),l=new RegExp("("+o+"\\s*:\\s*)([^;{}]+)(;)","g");return String(r||"").replace(l,"$1"+s+"$3")}function Pl(r,a){let s=o=>{if(o)try{o.documentElement?.style?.setProperty(r,a),o.body?.style?.setProperty(r,a),o.querySelector("[data-sve-template]")?.style?.setProperty(r,a)}catch{}};C("iframe").forEach(o=>{try{s(o.contentDocument)}catch{}})}function Re(r,a){let s=["css","head","html"],o=null;for(let S of s)if(hi(D(S),r)){o=S;break}if(!o)return!1;let l=D(o),h=$l(l,r,a),d=r+"-rgb",y=ed(a);return y&&hi(l,d)&&(h=$l(h,d,y)),h===l?!1:(ot(o,h),Pl(r,a),y&&hi(l,d)&&Pl(d,y),He(),!0)}function Qi(r){return String(u.defaults?.cssTokens?.[r]||"").trim()}function oe(r){let a=String(r?.type||"text").trim().toLowerCase();return a==="datetime-local"?"datetime":a==="checkbox"?"boolean":a}function td(r,a){return r?.readOnly===!0||r?.readonly===!0||r?.locked===!0||Yr(r,a)}function Nl(r){if(r&&Object.prototype.hasOwnProperty.call(r,"default"))return gt(r.default);let a=oe(r);return a==="boolean"?!1:""}function id(r){return(Array.isArray(r?.options)?r.options:[]).map(s=>{if(s&&typeof s=="object"&&!Array.isArray(s)){let o=s.value??s.id??s.key??"";return{value:String(o),label:String(s.label??s.name??o)}}return{value:String(s??""),label:String(s??"")}})}function rd(r){let a=[];return["min","max","step","maxlength","minlength","pattern"].forEach(s=>{r?.[s]!==void 0&&r?.[s]!==null&&String(r[s])!==""&&a.push(`${s}="${A(r[s])}"`)}),r?.placeholder&&a.push(`placeholder="${A(r.placeholder)}"`),a.join(" ")}function nd(r){let a=String(r?.help||r?.description||"").trim();return a?`
    ...
    `}function Zi(r){if(!r||typeof r!="object")return[];let a=u.contentFieldCache.get(r);if(a)return a;let s=(r.fields||[]).filter(o=>!(o.type==="repeater"&&Fl(o).length===0));return u.contentFieldCache.set(r,s),s}function pd(){if(u.contentSearchIndex)return u.contentSearchIndex;let r=new Map;return li().forEach(a=>{let s=Z(a),o="";try{o=JSON.stringify(a).toLowerCase()}catch{o=[s,a?.label||"",...Zi(a).flatMap(h=>[h?.label||"",h?.path||"",...(h?.fields||[]).flatMap(d=>[d?.label||"",d?.key||""])])].join(" ").toLowerCase()}r.set(s,o)}),u.contentSearchIndex=r,r}function Ji(r){r&&u.contentSectionHtmlCache.delete(String(r))}function Xi(r){let a=Z(r);if(!a)return Ol(r);if(u.contentSectionHtmlCache.has(a))return u.contentSectionHtmlCache.get(a);let s=Ol(r);return u.contentSectionHtmlCache.set(a,s),s}function jt(r){r&&(u.contentSectionUseTick+=1,r.dataset.contentUse=String(u.contentSectionUseTick))}function ln(r){if(!r)return;let a=C("[data-section-card]",r).filter(o=>k("[data-section-body]",o)?.dataset.loaded==="1"),s=a.length-u.contentMaxMountedSections;s<=0||a.filter(o=>!o.classList.contains("open")).sort((o,l)=>Number(o.dataset.contentUse||0)-Number(l.dataset.contentUse||0)).slice(0,s).forEach(o=>{let l=k("[data-section-body]",o);l&&(l.replaceChildren(),l.dataset.loaded="0")})}function hd(){if(u.contentPrewarmScheduled||!u.config||!Wi())return;let r=li();if(!r.length)return;u.contentPrewarmScheduled=!0;let a=s=>{u.contentPrewarmScheduled=!1,u.contentPrewarmHandle=null;let o=2;for(;u.contentPrewarmCursor<r.length&&o>0;){let l=r[u.contentPrewarmCursor++],h=Z(l);if(h&&!u.contentSectionHtmlCache.has(h)&&Xi(l),o-=1,s&&!s.didTimeout&&typeof s.timeRemaining=="function"&&s.timeRemaining()<5)break}u.contentPrewarmCursor<r.length&&(u.contentPrewarmScheduled=!0,u.contentPrewarmHandle=Ft(a,1200))};u.contentPrewarmHandle=Ft(a,1200)}function Ol(r){let a=Zi(r),s=new Map(hn(r).map(d=>[d.path,d])),o=[],l="",h=d=>{let y=String(d||"").trim();return!y||y===l?"":(l=y,`
      ...
      ${s.map(o=>{let l=Z(o),h=o.label||l,d=Bt(o),y=!o.visiblePath||B(u.config,o.visiblePath)!==!1,S=Zi(o),w=!u.search&&u.contentOpenSections.has(l);return`
            <article
...
`).trim();return a!==r.trim()?(ot("css",a),!0):!1}function bd(r){if(!r||!u.config)return;Array.from(r.querySelectorAll('[data-sve-type="image"][data-sve-field]')).forEach(s=>{let o=s.getAttribute("data-sve-field");if(!o)return;let l=Ut(o),h=s.closest("[data-sve-image-wrapper]"),d=h||s,y=l.align==="left"?"0":"auto",S=l.align==="right"?"0":"auto";h?(h.style.display=l.hidden?"none":"",h.style.width=l.width+"%",h.style.maxWidth="100%",h.style.marginLeft=y,h.style.marginRight=S,s.style.width="100%"):(s.style.display=l.hidden?"none":"",s.style.width=l.width+"%",s.style.maxWidth="100%",s.style.marginLeft=y,s.style.marginRight=S),l.fit==="auto"?s.style.removeProperty("object-fit"):s.style.objectFit=l.fit;let w=Xp[l.alignPos]||"";w?s.style.objectPosition=w:s.style.removeProperty("object-position"),s.style.height="100%"})}function kx(){pi.request({images:!0})}function xd(r){let a={"top left":`
    ...
    `}function yd(r){let a=Ut(r.path),s=l=>l==="left"?`
          <svg
            width="1em"
            height="1em"
            viewBox="0 0 21 20"
    ...
    `}function Cd(r,a){let s=B(u.config,r);if(!Array.isArray(s)||a<0||a>=s.length)return;let o=fd(r),l=pn(o),h=String(l?.key||"src"),d=Wl(),y={};for(let S=0;S<s.length;S++){let w=r+"."+S+"."+h;Object.prototype.hasOwnProperty.call(d,w)&&(y[S]=gt(d[w]))}s.splice(a,1),Object.keys(d).forEach(S=>{S.startsWith(r+".")&&S.endsWith("."+h)&&delete d[S]});for(let S=0;S<s.length;S++){let w=S<a?S:S+1,E=y[w];E&&(d[r+"."+S+"."+h]=E)}md(),Te("Foto gallery dihapus"),le()}function Ed(r){u.config&&(Ue(u.config,r,""),Ht(r,{hidden:!0}),Te("Gambar dihapus"),le())}function dn(r){let a=B(u.config,r.path)||"",s=Ut(r.path),l=`
...
`,ot("head",d)}let h=Array.from(document.querySelectorAll('link[id^="'+e+'-font-link"]'));r.forEach((d,y)=>{let S=y===0?e+"-font-link":e+"-font-link-"+y,w=document.getElementById(S);w||(w=document.createElement("link"),w.id=S,w.rel="stylesheet",document.head.appendChild(w)),w.href=d}),h.forEach(d=>{r.includes(d.href)||d.remove()}),Kl()}async function rr(r){let a=k("#"+e+"-"+r+"-font");if(!a)return;let s=$d(a.value);if(!s.valid||!s.family)return;let o=s.family;a.value=o;let l=await Nd(o,r);if(!l.ok){l.reason==="stylesheet"||l.reason==="font-file"||l.reason;return}let h=r==="heading"?"--sve-font-heading":"--sve-font-body",d=r==="heading"?"--sve-heading-weight":"--sve-body-weight";l.normalizedWeight&&l.weight&&Re(d,l.weight),Ue(u.config,"editorStyle.googleFonts."+r,o),Te(),Re(h,`"${o}", ${Rd(o,r)}`),Dd(r,!1),ir(),Kl(),He()}function nr(r,a,s=!0,o=!1){let l=String(r||"").trim(),h=s&&l&&!a.includes(l)?[l,...a]:[...a];return o&&(h=[...new Set(h)].sort((d,y)=>{let S=Number.parseFloat(d),w=Number.parseFloat(y);return Number.isFinite(S)&&Number.isFinite(w)?S-w:String(d).localeCompare(String(y))})),h.map((d,y)=>{let S=a.includes(l)||s?d===l:y===0;return`
      ...
      `;if(r.type==="weight"){let s=Fd(r.variable),o=s?Od(s):Y,l=s?tr(s,a):a;return`
        <select
          class="style-select"
          data-style-var="${A(r.variable)}"
        >
...
`)});return or={sources:r,report:s},s}function rc(){let r=ic();if(u.schema?.template?.type==="custom-page"){let F=Wd();return F.blockers=[...new Set([...r.blockers,...F.blockers])],F.blockers.length&&(F.status="BLOCKER"),F}let a=[...r.blockers],s=[],o=[],l=F=>a.push(F),h=F=>s.push(F),d=F=>o.push(F);if(u.config?d("CONFIG terbaca sebagai static object"):l("CONFIG tidak terbaca"),u.schema?d("SVE_SCHEMA eksplisit tersedia"):l("SVE_SCHEMA wajib eksplisit; HTML fallback bukan Strict PASS"),u.config)try{JSON.stringify(u.config),d("CONFIG JSON-compatible")}catch{l("CONFIG tidak dapat diserialisasi dengan aman")}let y=Array.isArray(u.schema?.sections)?u.schema.sections:[],S=y.map(Z).filter(Boolean),w=new Set(S);S.length!==w.size&&l("SVE_SCHEMA memiliki duplicate section id"),Ze.forEach(F=>{w.has(F)||l("Canonical section hilang: "+F)}),Ze.every(F=>w.has(F))&&d(Ze.length+" canonical sections tersedia");let E=Array.isArray(u.config?.sectionOrder)?u.config.sectionOrder:[],T=new Set(E);E.length!==T.size&&l("CONFIG.sectionOrder memiliki duplicate id"),Ze.forEach(F=>{T.has(F)||l("sectionOrder belum memuat: "+F)}),E[0]&&E[0]!=="cover"&&l("Cover wajib menjadi section pertama"),Ze.filter(F=>F!=="cover").forEach(F=>{typeof B(u.config,"sections."+F)!="boolean"&&l("Boolean visibility tidak valid: sections."+F)}),y.forEach(F=>{let Ie=Z(F);Ie==="cover"?(F.locked!==!0||F.canHide!==!1)&&l("Cover harus locked dan canHide:false"):F.visiblePath&&!mt(F.visiblePath)&&l("Unsafe visiblePath pada section "+Ie),sr(F.fields,We=>{let vi=oe(We);ai.has(vi)||l("Field type tidak didukung: "+vi+" ("+(We.path||We.key||Ie)+")"),We.path&&!mt(We.path)&&l("Unsafe field path: "+We.path),(vi==="repeater"||vi==="repeater-image")&&!Array.isArray(We.fields)&&l("Repeater tanpa fields[]: "+(We.path||Ie)),vi==="repeater"&&(We.fields||[]).forEach(pc=>{let hc=oe(pc);(hc==="repeater"||hc==="repeater-image")&&l("Nested repeater tidak diizinkan: "+(We.path||Ie)),pc.key||l("Repeater subfield tanpa stable key: "+(We.path||Ie))})})});let L=["html","css","js","head"].map(D).join(`
`);/\beval\s*\(/.test(L)&&l("eval() terdeteksi"),/\bnew\s+Function\s*\(/.test(L)&&l("new Function() terdeteksi"),/javascript\s*:/i.test(L)&&l("javascript: URL terdeteksi"),/https?:\/\/[^\s"']*scalev\.(?:com|id)\/api\//i.test(L)&&l("Private Scalev API URL terdeteksi"),/(service[_-]?role|database[_-]?password|private[_-]?api[_-]?key|secret[_-]?token)\s*[:=]/i.test(L)&&l("Kemungkinan secret/private credential terdeteksi"),tc(l,h,d);let G=D("js");/\bconst\s+CONFIG\s*=/.test(G)||s.push("CONFIG strict canonical sebaiknya memakai const"),/\bconst\s+SVE_SCHEMA\s*=/.test(G)||s.push("SVE_SCHEMA strict canonical sebaiknya memakai const");let ue=B(u.config,"sections.rsvp")===!0,At=B(u.config,"sections.guestbook")===!0,X=String(B(u.config,"rsvp.endpoint")||""),ze=B(u.config,"rsvp.enabled"),_e=!!X||ze!==void 0;if(ue)if(_e)ze!==!0&&l("RSVP & Ucapan visible tetapi rsvp.enabled bukan true"),/^https:\/\//i.test(X)||l("RSVP & Ucapan membutuhkan endpoint HTTPS");else{let F=String(B(u.config,"extensions.rsvpBackend.mode")||"none");if(F!=="none"&&F!=="external"&&l("RSVP backend mode harus none atau external"),F==="external"){let Ie=String(B(u.config,"extensions.rsvpBackend.endpoint")||"");/^https:\/\//i.test(Ie)||l("RSVP external membutuhkan endpoint HTTPS")}else s.push("RSVP backend belum dikonfigurasi; public runtime wajib fail-closed")}if(At){let F=B(u.config,"guestbook.enabled"),Ie=String(B(u.config,"guestbook.endpoint")||"");F!==!0&&l("Ucapan & Doa legacy visible tetapi guestbook.enabled bukan true"),/^https:\/\//i.test(Ie)||l("Ucapan & Doa legacy visible tetapi endpoint HTTPS belum valid")}let bt=on(),yi=se.map(F=>F.variable).filter(F=>!hi(bt,F));yi.length?l("Typography role tokens belum lengkap: "+yi.join(", ")):d("Semua typography role tokens tersedia"),/(?:\.svw-(?:cover-names|heading|quote-text|person-name|item-title|date-display|count\s+strong|gallery-caption|event-meta|field\s+label|footer-brand|footer-creator|footer-note|btn|kicker))[^\{]*\{[^\}]*font-size\s*:\s*(?!var\()/is.test(bt)&&h("Terdeteksi typography editorial hardcoded; map seluruh teks ke role token --sve-*.");let uc=ec();return Object.values(uc).reduce((F,Ie)=>F+Ie.length,0)?s.push("External origin terdeteksi; salin CSP manifest ke Scalev Security"):d("Tidak ada external origin wajib dari scanner"),{status:a.length?"BLOCKER":s.length?"WARNING":"PASS",blockers:a,warnings:s,passes:o,csp:uc}}function Gd(r){return r==="PASS"?`
    ...
    `}function qd(){if(!u.config)return fi();let r=rc(),a=(l,h)=>l.length?`<ul>${l.map(d=>`<li>${A(d)}</li>`).join("")}</ul>`:`<p class="compat-empty">${A(h)}</p>`,s=r.status==="PASS"?"Siap":r.status==="WARNING"?"Perlu dicek":"Masalah",o=r.status==="PASS"?"Semua siap":r.status==="WARNING"?"Perlu diperiksa":"Perlu diperbaiki";return`
