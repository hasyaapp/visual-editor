...
`)}function Xh(r){let a=String(r||"").trim(),s=a.match(/^#([0-9a-f]{3}|[0-9a-f]{6})$/i);if(s){let l=s[1];l.length===3&&(l=l.split("").map(d=>d+d).join(""));let h=parseInt(l,16);return[h>>16&255,h>>8&255,h&255].join(", ")}let o=a.match(/^rgba?\(\s*(\d{1,3})\s*[,\s]\s*(\d{1,3})\s*[,\s]\s*(\d{1,3})/i);return o?[o[1],o[2],o[3]].join(", "):""}function $l(r,a,s){let o=a.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),l=new RegExp("("+o+"\\s*:\\s*)([^;{}]+)(;)","g");return String(r||"").replace(l,"$1"+s+"$3")}function Pl(r,a){let s=o=>{if(o)try{o.documentElement?.style?.setProperty(r,a),o.body?.style?.setProperty(r,a),o.querySelector("[data-sve-template]")?.style?.setProperty(r,a)}catch{}};C("iframe").forEach(o=>{try{s(o.contentDocument)}catch{}})}function Re(r,a){let s=["css","head","html"],o=null;for(let S of s)if(hi(V(S),r)){o=S;break}if(!o)return!1;let l=V(o),h=$l(l,r,a),d=r+"-rgb",y=Xh(a);return y&&hi(l,d)&&(h=$l(h,d,y)),h===l?!1:(ot(o,h),Pl(r,a),y&&hi(l,d)&&Pl(d,y),He(),!0)}function Qi(r){return String(u.defaults?.cssTokens?.[r]||"").trim()}function oe(r){let a=String(r?.type||"text").trim().toLowerCase();return a==="datetime-local"?"datetime":a==="checkbox"?"boolean":a}function ed(r,a){return r?.readOnly===!0||r?.readonly===!0||r?.locked===!0||Yr(r,a)}function Nl(r){if(r&&Object.prototype.hasOwnProperty.call(r,"default"))return gt(r.default);let a=oe(r);return a==="boolean"?!1:""}function td(r){return(Array.isArray(r?.options)?r.options:[]).map(s=>{if(s&&typeof s=="object"&&!Array.isArray(s)){let o=s.value??s.id??s.key??"";return{value:String(o),label:String(s.label??s.name??o)}}return{value:String(s??""),label:String(s??"")}})}function id(r){let a=[];return["min","max","step","maxlength","minlength","pattern"].forEach(s=>{r?.[s]!==void 0&&r?.[s]!==null&&String(r[s])!==""&&a.push(`${s}="${A(r[s])}"`)}),r?.placeholder&&a.push(`placeholder="${A(r.placeholder)}"`),a.join(" ")}function rd(r){let a=String(r?.help||r?.description||"").trim();return a?`
    ...
    `}function Zi(r){if(!r||typeof r!="object")return[];let a=u.contentFieldCache.get(r);if(a)return a;let s=(r.fields||[]).filter(o=>!(o.type==="repeater"&&Fl(o).length===0));return u.contentFieldCache.set(r,s),s}function ud(){if(u.contentSearchIndex)return u.contentSearchIndex;let r=new Map;return li().forEach(a=>{let s=Z(a),o="";try{o=JSON.stringify(a).toLowerCase()}catch{o=[s,a?.label||"",...Zi(a).flatMap(h=>[h?.label||"",h?.path||"",...(h?.fields||[]).flatMap(d=>[d?.label||"",d?.key||""])])].join(" ").toLowerCase()}r.set(s,o)}),u.contentSearchIndex=r,r}function Ji(r){r&&u.contentSectionHtmlCache.delete(String(r))}function Xi(r){let a=Z(r);if(!a)return Ol(r);if(u.contentSectionHtmlCache.has(a))return u.contentSectionHtmlCache.get(a);let s=Ol(r);return u.contentSectionHtmlCache.set(a,s),s}function jt(r){r&&(u.contentSectionUseTick+=1,r.dataset.contentUse=String(u.contentSectionUseTick))}function ln(r){if(!r)return;let a=C("[data-section-card]",r).filter(o=>k("[data-section-body]",o)?.dataset.loaded==="1"),s=a.length-u.contentMaxMountedSections;s<=0||a.filter(o=>!o.classList.contains("open")).sort((o,l)=>Number(o.dataset.contentUse||0)-Number(l.dataset.contentUse||0)).slice(0,s).forEach(o=>{let l=k("[data-section-body]",o);l&&(l.replaceChildren(),l.dataset.loaded="0")})}function pd(){if(u.contentPrewarmScheduled||!u.config||!Wi())return;let r=li();if(!r.length)return;u.contentPrewarmScheduled=!0;let a=s=>{u.contentPrewarmScheduled=!1,u.contentPrewarmHandle=null;let o=2;for(;u.contentPrewarmCursor<r.length&&o>0;){let l=r[u.contentPrewarmCursor++],h=Z(l);if(h&&!u.contentSectionHtmlCache.has(h)&&Xi(l),o-=1,s&&!s.didTimeout&&typeof s.timeRemaining=="function"&&s.timeRemaining()<5)break}u.contentPrewarmCursor<r.length&&(u.contentPrewarmScheduled=!0,u.contentPrewarmHandle=Ft(a,1200))};u.contentPrewarmHandle=Ft(a,1200)}function Ol(r){let a=Zi(r),s=new Map(hn(r).map(d=>[d.path,d])),o=[],l="",h=d=>{let y=String(d||"").trim();return!y||y===l?"":(l=y,`
      ...
      ${s.map(o=>{let l=Z(o),h=o.label||l,d=Bt(o),y=!o.visiblePath||D(u.config,o.visiblePath)!==!1,S=Zi(o),w=!u.search&&u.contentOpenSections.has(l);return`
            <article
...
`).trim();return a!==r.trim()?(ot("css",a),!0):!1}function gd(r){if(!r||!u.config)return;Array.from(r.querySelectorAll('[data-sve-type="image"][data-sve-field]')).forEach(s=>{let o=s.getAttribute("data-sve-field");if(!o)return;let l=Ut(o),h=s.closest("[data-sve-image-wrapper]"),d=h||s,y=l.align==="left"?"0":"auto",S=l.align==="right"?"0":"auto";h?(h.style.display=l.hidden?"none":"",h.style.width=l.width+"%",h.style.maxWidth="100%",h.style.marginLeft=y,h.style.marginRight=S,s.style.width="100%"):(s.style.display=l.hidden?"none":"",s.style.width=l.width+"%",s.style.maxWidth="100%",s.style.marginLeft=y,s.style.marginRight=S),l.fit==="auto"?s.style.removeProperty("object-fit"):s.style.objectFit=l.fit;let w=Zp[l.alignPos]||"";w?s.style.objectPosition=w:s.style.removeProperty("object-position"),s.style.height="100%"})}function vx(){pi.request({images:!0})}function bd(r){let a={"top left":`
    ...
    `}function xd(r){let a=Ut(r.path),s=l=>l==="left"?`
          <svg
            width="1em"
            height="1em"
            viewBox="0 0 21 20"
    ...
    `}function wd(r,a){let s=D(u.config,r);if(!Array.isArray(s)||a<0||a>=s.length)return;let o=dd(r),l=pn(o),h=String(l?.key||"src"),d=Wl(),y={};for(let S=0;S<s.length;S++){let w=r+"."+S+"."+h;Object.prototype.hasOwnProperty.call(d,w)&&(y[S]=gt(d[w]))}s.splice(a,1),Object.keys(d).forEach(S=>{S.startsWith(r+".")&&S.endsWith("."+h)&&delete d[S]});for(let S=0;S<s.length;S++){let w=S<a?S:S+1,E=y[w];E&&(d[r+"."+S+"."+h]=E)}fd(),Te("Foto gallery dihapus"),ce()}function Cd(r){u.config&&(Ue(u.config,r,""),Ht(r,{hidden:!0}),Te("Gambar dihapus"),ce())}function dn(r){let a=D(u.config,r.path)||"",s=Ut(r.path),l=`
          ...
          `;return`
            ...
            >
              ...
              ${r.gallery?(()=>{let d=r.path.replace(/\.src$/,".alt"),y=D(u.config,d)||"";return`
    ...
    `}function _d(){return u.config?Ad()?[...new Set(_.map(a=>a[0]))].map(a=>`
            <div class="group">
              <div class="group-title">
                ${a}
              </div>

              ${_.filter(s=>s[0]===a).map(([,s,o])=>Td(s,o)).join("")}
            </div>
          `).join("")+`
        <button
          type="button"
...
`,ot("head",d)}let h=Array.from(document.querySelectorAll('link[id^="'+e+'-font-link"]'));r.forEach((d,y)=>{let S=y===0?e+"-font-link":e+"-font-link-"+y,w=document.getElementById(S);w||(w=document.createElement("link"),w.id=S,w.rel="stylesheet",document.head.appendChild(w)),w.href=d}),h.forEach(d=>{r.includes(d.href)||d.remove()}),Kl()}async function rr(r){let a=k("#"+e+"-"+r+"-font");if(!a)return;let s=Ld(a.value);if(!s.valid||!s.family)return;let o=s.family;a.value=o;let l=await Pd(o,r);if(!l.ok){l.reason==="stylesheet"||l.reason==="font-file"||l.reason;return}let h=r==="heading"?"--sve-font-heading":"--sve-font-body",d=r==="heading"?"--sve-heading-weight":"--sve-body-weight";l.normalizedWeight&&l.weight&&Re(d,l.weight),Ue(u.config,"editorStyle.googleFonts."+r,o),Te(),Re(h,`"${o}", ${Nd(o,r)}`),Od(r,!1),ir(),Kl(),He()}function nr(r,a,s=!0,o=!1){let l=String(r||"").trim(),h=s&&l&&!a.includes(l)?[l,...a]:[...a];return o&&(h=[...new Set(h)].sort((d,y)=>{let S=Number.parseFloat(d),w=Number.parseFloat(y);return Number.isFinite(S)&&Number.isFinite(w)?S-w:String(d).localeCompare(String(y))})),h.map((d,y)=>{let S=a.includes(l)||s?d===l:y===0;return`
          ...
          `}).join("")}function Dd(r){let a=be(r.variable)||r.fallback;if(r.type==="size")return`
        <select
          class="style-select"
      ...
      `;if(r.type==="weight"){let s=Rd(r.variable),o=s?Md(s):Y,l=s?tr(s,a):a;return`
        <select
          class="style-select"
          data-style-var="${A(r.variable)}"
        >
    ...
    `}function sr(r,a){(Array.isArray(r)?r:[]).forEach(s=>{a(s),oe(s)==="repeater"&&sr(s.fields,a),oe(s)==="repeater-image"&&sr(s.fields,a)})}function ec(){let r={connect_src:new Set,img_src:new Set,media_src:new Set,font_src:new Set,script_src:new Set,style_src:new Set,frame_src:new Set,worker_src:new Set,manifest_src:new Set},a={html:V("html"),css:V("css"),js:V("js"),head:V("head")},s=(S,w)=>{try{let E=new URL(w,location.origin);if(E.protocol!=="https:"&&E.protocol!=="http:")return;let T=E.origin;if(T===location.origin)return;r[S]?.add(T)}catch{}},o=(S,w)=>{let E=/https?:\/\/[^\s"'<>`)\\]+/g;(String(S||"").match(E)||[]).forEach(T=>s(w,T))};try{let S=new DOMParser().parseFromString(a.html||"","text/html");S.querySelectorAll("img[src], source[src], source[srcset]").forEach(w=>{s("img_src",w.getAttribute("src")||w.getAttribute("srcset")||"")}),S.querySelectorAll("audio[src], video[src]").forEach(w=>s("media_src",w.getAttribute("src")||"")),S.querySelectorAll("iframe[src]").forEach(w=>s("frame_src",w.getAttribute("src")||"")),S.querySelectorAll("script[src]").forEach(w=>s("script_src",w.getAttribute("src")||"")),S.querySelectorAll('link[rel="stylesheet"][href]').forEach(w=>s("style_src",w.getAttribute("href")||"")),S.querySelectorAll('link[rel="manifest"][href]').forEach(w=>s("manifest_src",w.getAttribute("href")||""))}catch{}let l=/url\(\s*["']?(https?:\/\/[^)"']+)["']?\s*\)/g,h;for(;h=l.exec((a.css||"")+`
`+(a.head||""));){let S=h[1];/fonts\.gstatic\.com/i.test(S)?s("font_src",S):s("img_src",S)}o(a.head,"style_src");let d=JSON.stringify(u.config||{}),y=D(u.config,"guestbook.endpoint");return y&&s("connect_src",y),["rsvp.endpoint","extensions.rsvpBackend.endpoint"].forEach(S=>{let w=D(u.config,S);w&&s("connect_src",w)}),(d.match(/https?:\/\/[^"\\]+/g)||[]).forEach(S=>{/youtube\.com|youtu\.be/i.test(S)?s("frame_src",S):/\.(?:mp3|m4a|wav|ogg|mp4|webm)(?:\?|$)/i.test(S)?s("media_src",S):/\.(?:woff2?|ttf|otf)(?:\?|$)/i.test(S)?s("font_src",S):/\.(?:png|jpe?g|webp|gif|svg|avif)(?:\?|$)/i.test(S)&&s("img_src",S)}),/fonts\.googleapis\.com/i.test(a.head||"")&&(r.style_src.add("https://fonts.googleapis.com"),r.font_src.add("https://fonts.gstatic.com")),Object.fromEntries(Object.entries(r).map(([S,w])=>[S,Array.from(w).sort()]))}function Hd(){return{"Body HTML":V("html"),CSS:V("css"),JavaScript:V("js"),"Additional Head":V("head"),CONFIG:JSON.stringify(u.config||{})}}function tc(r,a,s){let o=Hd(),l=Yp(o);l.length?r("Gambar base64 terdeteksi di "+rl(l)+"; upload gambar ke hosting lalu pakai URL https"):s("Tidak ada gambar base64");let h=Qp(o);h.length&&a("Data URI berukuran besar di "+rl(h)+"; pertimbangkan pindah ke file hosting")}function zd(){let r=[],a=[],s=[],o=X=>r.push(X),l=X=>a.push(X),h=X=>s.push(X);if(u.config?h("CONFIG terbaca sebagai static object"):o("CONFIG tidak terbaca"),u.schema?h("SVE_SCHEMA custom page tersedia"):o("SVE_SCHEMA wajib eksplisit"),u.config)try{JSON.stringify(u.config),h("CONFIG JSON-compatible")}catch{o("CONFIG tidak dapat diserialisasi dengan aman")}let d=Array.isArray(u.schema?.sections)?u.schema.sections:[],y=d.map(Z).filter(Boolean),S=new Set(y);d.length||o("SVE_SCHEMA custom page belum memiliki section"),y.length!==S.size&&o("SVE_SCHEMA memiliki duplicate section id");let w=Array.isArray(u.config?.sectionOrder)?u.config.sectionOrder:[],E=new Set(w);w.length!==E.size&&o("CONFIG.sectionOrder memiliki duplicate id"),y.forEach(X=>{E.has(X)||o("sectionOrder belum memuat: "+X)}),d.forEach(X=>{let ze=Z(X);X.visiblePath&&(mt(X.visiblePath)||o("Unsafe visiblePath pada section "+ze),u.config&&typeof D(u.config,X.visiblePath)!="boolean"&&o("Visibility path harus boolean pada section "+ze)),sr(X.fields,_e=>{let bt=oe(_e);ai.has(bt)||o("Field type tidak didukung: "+bt+" ("+(_e.path||_e.key||ze)+")"),_e.path&&!mt(_e.path)&&o("Unsafe field path: "+_e.path),(bt==="repeater"||bt==="repeater-image")&&!Array.isArray(_e.fields)&&o("Repeater tanpa fields[]: "+(_e.path||ze)),bt==="repeater"&&(_e.fields||[]).forEach(yi=>{let yn=oe(yi);(yn==="repeater"||yn==="repeater-image")&&o("Nested repeater tidak diizinkan: "+(_e.path||ze)),yi.key||o("Repeater subfield tanpa stable key: "+(_e.path||ze))})})});let T=["html","css","js","head"].map(V).join(`
`);/\beval\s*\(/.test(T)&&o("eval() terdeteksi"),/\bnew\s+Function\s*\(/.test(T)&&o("new Function() terdeteksi"),/javascript\s*:/i.test(T)&&o("javascript: URL terdeteksi"),/https?:\/\/[^\s"']*scalev\.(?:com|id)\/api\//i.test(T)&&o("Private Scalev API URL terdeteksi"),/(service[_-]?role|database[_-]?password|private[_-]?api[_-]?key|secret[_-]?token)\s*[:=]/i.test(T)&&o("Kemungkinan secret/private credential terdeteksi"),tc(o,l,h);let L=on(),G=se.map(X=>X.variable).filter(X=>!hi(L,X));G.length?o("Typography role tokens belum lengkap: "+G.join(", ")):h("Semua typography role tokens tersedia");let ue=ec();return Object.values(ue).reduce((X,ze)=>X+ze.length,0)&&l("External origin terdeteksi; salin CSP manifest ke Scalev Security"),h("Custom page aktif; validasi "+Ze.length+" section wedding dilewati"),{status:r.length?"BLOCKER":a.length?"WARNING":"PASS",blockers:r,warnings:a,passes:s,csp:ue}}let or=null;function ic(){let r=["html","css","js","head"].map(V);if(or&&r.every((o,l)=>o===or.sources[l]))return or.report;let a=new DOMParser().parseFromString(r[0],"text/html");a.head.insertAdjacentHTML("beforeend",r[3]);let s=tl({doc:a,scripts:[r[2],...Array.from(a.querySelectorAll("script"),o=>o.textContent)].filter(Boolean),css:r[1]+`
`+Array.from(a.querySelectorAll("style"),o=>o.textContent).join(`
`)});return or={sources:r,report:s},s}function Wd(){let r=ic();if(u.schema?.template?.type==="custom-page"){let F=zd();return F.blockers=[...new Set([...r.blockers,...F.blockers])],F.blockers.length&&(F.status="BLOCKER"),F}let a=[...r.blockers],s=[],o=[],l=F=>a.push(F),h=F=>s.push(F),d=F=>o.push(F);if(u.config?d("CONFIG terbaca sebagai static object"):l("CONFIG tidak terbaca"),u.schema?d("SVE_SCHEMA eksplisit tersedia"):l("SVE_SCHEMA wajib eksplisit; HTML fallback bukan Strict PASS"),u.config)try{JSON.stringify(u.config),d("CONFIG JSON-compatible")}catch{l("CONFIG tidak dapat diserialisasi dengan aman")}let y=Array.isArray(u.schema?.sections)?u.schema.sections:[],S=y.map(Z).filter(Boolean),w=new Set(S);S.length!==w.size&&l("SVE_SCHEMA memiliki duplicate section id"),Ze.forEach(F=>{w.has(F)||l("Canonical section hilang: "+F)}),Ze.every(F=>w.has(F))&&d(Ze.length+" canonical sections tersedia");let E=Array.isArray(u.config?.sectionOrder)?u.config.sectionOrder:[],T=new Set(E);E.length!==T.size&&l("CONFIG.sectionOrder memiliki duplicate id"),Ze.forEach(F=>{T.has(F)||l("sectionOrder belum memuat: "+F)}),E[0]&&E[0]!=="cover"&&l("Cover wajib menjadi section pertama"),D(u.config,"invitation.isDemo")===!0&&h("Mode Demo AKTIF \u2014 RSVP tamu tidak dikirim ke server. Matikan sebelum dipakai klien."),D(u.config,"invitation.isExclusive")===!0&&h("Undangan Khusus AKTIF \u2014 halaman hanya terbuka dengan link bertoken."),Ze.filter(F=>F!=="cover").forEach(F=>{typeof D(u.config,"sections."+F)!="boolean"&&l("Boolean visibility tidak valid: sections."+F)}),y.forEach(F=>{let Ie=Z(F);Ie==="cover"?(F.locked!==!0||F.canHide!==!1)&&l("Cover harus locked dan canHide:false"):F.visiblePath&&!mt(F.visiblePath)&&l("Unsafe visiblePath pada section "+Ie),sr(F.fields,We=>{let vi=oe(We);ai.has(vi)||l("Field type tidak didukung: "+vi+" ("+(We.path||We.key||Ie)+")"),We.path&&!mt(We.path)&&l("Unsafe field path: "+We.path),(vi==="repeater"||vi==="repeater-image")&&!Array.isArray(We.fields)&&l("Repeater tanpa fields[]: "+(We.path||Ie)),vi==="repeater"&&(We.fields||[]).forEach(cc=>{let uc=oe(cc);(uc==="repeater"||uc==="repeater-image")&&l("Nested repeater tidak diizinkan: "+(We.path||Ie)),cc.key||l("Repeater subfield tanpa stable key: "+(We.path||Ie))})})});let L=["html","css","js","head"].map(V).join(`
`);/\beval\s*\(/.test(L)&&l("eval() terdeteksi"),/\bnew\s+Function\s*\(/.test(L)&&l("new Function() terdeteksi"),/javascript\s*:/i.test(L)&&l("javascript: URL terdeteksi"),/https?:\/\/[^\s"']*scalev\.(?:com|id)\/api\//i.test(L)&&l("Private Scalev API URL terdeteksi"),/(service[_-]?role|database[_-]?password|private[_-]?api[_-]?key|secret[_-]?token)\s*[:=]/i.test(L)&&l("Kemungkinan secret/private credential terdeteksi"),tc(l,h,d);let G=V("js");/\bconst\s+CONFIG\s*=/.test(G)||s.push("CONFIG strict canonical sebaiknya memakai const"),/\bconst\s+SVE_SCHEMA\s*=/.test(G)||s.push("SVE_SCHEMA strict canonical sebaiknya memakai const");let ue=D(u.config,"sections.rsvp")===!0,At=D(u.config,"sections.guestbook")===!0,X=String(D(u.config,"rsvp.endpoint")||""),ze=D(u.config,"rsvp.enabled"),_e=!!X||ze!==void 0;if(ue)if(_e)ze!==!0&&l("RSVP & Ucapan visible tetapi rsvp.enabled bukan true"),/^https:\/\//i.test(X)||l("RSVP & Ucapan membutuhkan endpoint HTTPS");else{let F=String(D(u.config,"extensions.rsvpBackend.mode")||"none");if(F!=="none"&&F!=="external"&&l("RSVP backend mode harus none atau external"),F==="external"){let Ie=String(D(u.config,"extensions.rsvpBackend.endpoint")||"");/^https:\/\//i.test(Ie)||l("RSVP external membutuhkan endpoint HTTPS")}else s.push("RSVP backend belum dikonfigurasi; public runtime wajib fail-closed")}if(At){let F=D(u.config,"guestbook.enabled"),Ie=String(D(u.config,"guestbook.endpoint")||"");F!==!0&&l("Ucapan & Doa legacy visible tetapi guestbook.enabled bukan true"),/^https:\/\//i.test(Ie)||l("Ucapan & Doa legacy visible tetapi endpoint HTTPS belum valid")}let bt=on(),yi=se.map(F=>F.variable).filter(F=>!hi(bt,F));yi.length?l("Typography role tokens belum lengkap: "+yi.join(", ")):d("Semua typography role tokens tersedia"),/(?:\.svw-(?:cover-names|heading|quote-text|person-name|item-title|date-display|count\s+strong|gallery-caption|event-meta|field\s+label|footer-brand|footer-creator|footer-note|btn|kicker))[^\{]*\{[^\}]*font-size\s*:\s*(?!var\()/is.test(bt)&&h("Terdeteksi typography editorial hardcoded; map seluruh teks ke role token --sve-*.");let lc=ec();return Object.values(lc).reduce((F,Ie)=>F+Ie.length,0)?s.push("External origin terdeteksi; salin CSP manifest ke Scalev Security"):d("Tidak ada external origin wajib dari scanner"),{status:a.length?"BLOCKER":s.length?"WARNING":"PASS",blockers:a,warnings:s,passes:o,csp:lc}}function Gd(r){return r==="PASS"?`
    ...
    `}function qd(){if(!u.config)return fi();let r=Wd(),a=(l,h)=>l.length?`<ul>${l.map(d=>`<li>${A(d)}</li>`).join("")}</ul>`:`<p class="compat-empty">${A(h)}</p>`,s=r.status==="PASS"?"Siap":r.status==="WARNING"?"Perlu dicek":"Masalah",o=r.status==="PASS"?"Semua siap":r.status==="WARNING"?"Perlu diperiksa":"Perlu diperbaiki";return`
    ...
    `}function Kd(r){let a=[],s=new WeakSet,o=(l,h="CONFIG")=>{if(l!==null){if(typeof l=="object"){if(s.has(l)){a.push("Referensi berulang: "+h);return}s.add(l)}if(Array.isArray(l)){l.forEach((d,y)=>o(d,h+"."+y));return}if(typeof l=="object"){Object.keys(l).forEach(d=>{H.has(d)&&a.push("Forbidden key: "+h+"."+d),o(l[d],h+"."+d)});return}["string","number","boolean"].includes(typeof l)||a.push("Non-static value: "+h),typeof l=="number"&&!Number.isFinite(l)&&a.push("Non-finite number: "+h)}};o(r);try{JSON.parse(JSON.stringify(r))}catch{a.push("CONFIG gagal round-trip JSON")}return a}function Yd(){let r=u.templateLibrary,a=String(u.search||"").trim().toLowerCase(),s=r.templates.filter(h=>a?[h.name].join(" ").toLowerCase().includes(a):!0);r.status==="idle"&&bl().then(()=>{u.tab==="library"&&(u.uiPrepared=!1,ce())});let o=r.error?`
...
Perubahan Anda akan hilang dan tidak bisa dibatalkan.`;if(!window.confirm(E))return;clearTimeout(u.contentCommitTimer),u.contentCommitTimer=null,u.contentCommitMessage="",u.contentStateDirty=!1,Ll();return}if(s.target.closest("#"+e+"-team-key-save")){Gh();return}if(s.target.closest("#"+e+"-pin-peek")){sn("peek");return}if(s.target.closest("#"+e+"-pin-generate")){sn("generate");return}if(s.target.closest("#"+e+"-pin-copy")){Kh();return}if(s.target.closest("#"+e+"-pin-changekey")){qh();return}let S=s.target.closest(".section-head");if(S&&!s.target.closest(".switch-wrap, .section-actions, .section-move-controls, .section-drag-btn")){let w=S.closest("[data-section-card]");if(!w)return;let E=!w.classList.contains("open");w.classList.toggle("open",E);let T=w.dataset.sectionCard;E?cn(w):(u.contentOpenSections.delete(T),k(".chev",w)?.setAttribute("aria-expanded","false"),jt(w),ln(r))}}),r.addEventListener("input",s=>{let o=s.target;o instanceof HTMLElement&&o.matches("[data-field-path]")&&(o.tagName==="SELECT"||o.matches('input[type="checkbox"], input[type="radio"]')||nc(o)&&Bl())}),r.addEventListener("change",s=>{let o=s.target;if(o instanceof HTMLElement){if(o.matches("[data-visible-path]")){Ue(u.config,o.dataset.visiblePath,o.checked),Bl(o.checked?"Section ditampilkan":"Section disembunyikan");return}nc(o)&&ke("Konten diperbarui")}}),r.addEventListener("dragstart",s=>{let o=s.target.closest("[data-section-drag]");if(!o)return;if(o.disabled||o.getAttribute("draggable")!=="true"){s.preventDefault();return}ke();let l=o.closest("[data-section-card]");l&&(l.classList.add("dragging"),s.dataTransfer.effectAllowed="move",s.dataTransfer.setData("text/plain",l.dataset.sectionCard),typeof s.dataTransfer.setDragImage=="function"&&s.dataTransfer.setDragImage(l,24,24))}),r.addEventListener("dragend",a),r.addEventListener("dragover",s=>{let o=s.target.closest("[data-section-card]");if(!o)return;let l=s.dataTransfer?.getData("text/plain")||k(".section.dragging",r)?.dataset?.sectionCard||"",h=o.dataset.sectionCard;if(!l||l===h)return;let d=ve().find(w=>Z(w)===h);if(h!=="cover"&&!Bt(d))return;s.preventDefault(),s.dataTransfer.dropEffect="move";let y=o.getBoundingClientRect(),S=s.clientY<y.top+y.height/2?"before":"after";h==="cover"&&(S="after"),C(".section.drag-before, .section.drag-after",r).forEach(w=>{w!==o&&(w.classList.remove("drag-before","drag-after"),delete w.dataset.dropPlacement)}),o.dataset.dropPlacement=S,o.classList.toggle("drag-before",S==="before"),o.classList.toggle("drag-after",S==="after")}),r.addEventListener("dragleave",s=>{let o=s.target.closest("[data-section-card]");o&&(s.relatedTarget&&o.contains(s.relatedTarget)||(o.classList.remove("drag-before","drag-after"),delete o.dataset.dropPlacement))}),r.addEventListener("drop",s=>{let o=s.target.closest("[data-section-card]");if(!o)return;let l=s.dataTransfer.getData("text/plain"),h=o.dataset.sectionCard,d=o.dataset.dropPlacement||(h==="cover"?"after":"before");s.preventDefault(),a(),Ph(l,h,d)})}function tf(r){C("[data-library-import]",r).forEach(a=>{a.onclick=()=>{Th(a.dataset.libraryImport)}}),k("[data-library-clear]",r)?.addEventListener("click",_h),k("[data-library-refresh]",r)?.addEventListener("click",async()=>{await bl(!0),u.uiPrepared=!1,ce()})}function xi(r,a){let s=a+"Delegated";return r.dataset[s]==="1"?!1:(r.dataset[s]="1",!0)}function rf(r){if(u.tab==="library"){tf(r);return}if(u.tab==="content"){ac(r),nf(r);return}if(u.tab==="colors"){af(r);return}if(u.tab==="style"){sf(r);return}if(u.tab==="audio"){of(r);return}if(u.tab==="compatibility"){lf(r);return}ac(r)}function nf(r){if(!xi(r,"images"))return;r.addEventListener("click",s=>{let o=s.target.closest("[data-image-paste-path]");if(o){s.preventDefault(),s.stopPropagation(),Xd(r,o.dataset.imagePastePath);return}let l=s.target.closest("[data-image-delete-path]");if(l){Cd(l.dataset.imageDeletePath);return}let h=s.target.closest("[data-image-open-advance]");if(h){let E=h.dataset.imageOpenAdvance,T=k(`[data-image-card-path="${CSS.escape(E)}"]`,r),L=T?k(".image-advance",T):null;if(L){let G=!L.open;L.open=G,h.setAttribute("aria-expanded",String(G)),h.setAttribute("aria-label",G?"Tutup pengaturan gambar":"Buka pengaturan gambar"),h.title=G?"Tutup pengaturan gambar":"Pengaturan gambar",h.classList.toggle("active",G),G?L.scrollIntoView({block:"nearest",behavior:"smooth"}):h.closest(".image-card")?.scrollIntoView({block:"nearest",behavior:"smooth"})}return}let d=s.target.closest("[data-image-align-path]");if(d){let E=d.dataset.imageAlignPath,T=["left","center","right"].includes(d.dataset.imageAlign)?d.dataset.imageAlign:"center";Ht(E,{align:T}),Ki(),er(r,E);let L=k(`[data-image-path="${CSS.escape(E)}"]`,r);L&&mi(L,E);return}let y=s.target.closest("[data-image-fit-path]");if(y){let E=y.dataset.imageFitPath,T=nl.includes(y.dataset.imageFit)?y.dataset.imageFit:"auto";Ht(E,{fit:T}),Ki(),er(r,E);let L=k(`[data-image-path="${CSS.escape(E)}"]`,r);L&&mi(L,E);return}let S=s.target.closest("[data-image-alignpos-path]");if(S){let E=S.dataset.imageAlignposPath,T=qr.includes(S.dataset.imageAlignpos)?S.dataset.imageAlignpos:"default";Ht(E,{alignPos:T}),Ki(),er(r,E);let L=k(`[data-image-path="${CSS.escape(E)}"]`,r);L&&mi(L,E);return}let w=s.target.closest("[data-gallery-delete-index]");if(w){wd(w.dataset.galleryDeleteIndex,Number(w.dataset.galleryIndex));return}}),r.addEventListener("input",s=>{let o=s.target.dataset.imageWidthPath;if(o!==void 0){let h=k(`[data-image-width-number="${CSS.escape(o)}"]`,r);h&&(h.value=s.target.value);return}let l=s.target.dataset.imageWidthNumber;if(l!==void 0){let h=Math.max(0,Math.min(100,Number(s.target.value)||0)),d=k(`[data-image-width-path="${CSS.escape(l)}"]`,r);d&&(d.value=h);return}});let a=(s,o)=>{let l=Math.max(0,Math.min(100,Number(o)||0));Ht(s,{width:l}),Ki();let h=k(`[data-image-path="${CSS.escape(s)}"]`,r);h&&mi(h,s),er(r,s)};r.addEventListener("change",s=>{let o=s.target.dataset.imageWidthPath;if(o!==void 0){a(o,s.target.value);return}let l=s.target.dataset.imageWidthNumber;if(l!==void 0){a(l,s.target.value);return}let h=s.target.closest("[data-image-path]");if(!h)return;let d=h.dataset.imagePath,y=h.value.trim(),S=String(D(u.config,d)||"");if(y!==S){if(Gr(y)){h.value=S,rc(h);return}Ue(u.config,d,y),y&&Ht(d,{hidden:!1}),Te("Gambar diperbarui"),mi(h,d)}}),r.addEventListener("paste",s=>{let o=s.target.closest("[data-image-path]");o&&setTimeout(()=>{o.dispatchEvent(new Event("change",{bubbles:!0}))},0)})}function af(r){if(!xi(r,"colors"))return;let a=(o,l,h)=>{let d=o.value.trim();if(!d||!Ed(d)){if(h){let S=be(l);S&&(o.value=S)}return}Re(l,d);let y=k(`[data-color-var="${CSS.escape(l)}"]`,r);y&&(y.value=gi(d,y.value||"#000000"))},s=o=>{let l=Qi(o);if(!l)return;Re(o,l);let h=k(`[data-color-token-var="${CSS.escape(o)}"], [data-style-var="${CSS.escape(o)}"]`,r),d=k(`[data-color-var="${CSS.escape(o)}"]`,r);if(h){let y=h.tagName==="SELECT"?Array.from(h.options).map(S=>S.value):[];(!y.length||y.includes(l))&&(h.value=l)}d&&(d.value=gi(l,d.value))};r.addEventListener("click",o=>{let l=o.target.closest("[data-reset-token]");if(l){s(l.dataset.resetToken);return}if(o.target.closest("#"+e+"-reset-colors")){_.forEach(([,,h])=>{let d=be(h);d&&Re(h,Qi(h)||d)}),C("[data-color-token-var]",r).forEach(h=>{let d=h.dataset.colorTokenVar,y=be(d);y&&(h.value=y)}),C("[data-color-var]",r).forEach(h=>{h.value=gi(be(h.dataset.colorVar),h.value)});return}}),r.addEventListener("input",o=>{let l=o.target.dataset.colorTokenVar;if(l!==void 0){a(o.target,l,!1);return}let h=o.target.dataset.colorVar;if(h!==void 0){Re(h,o.target.value);let d=k(`[data-color-token-var="${CSS.escape(h)}"]`,r);d&&(d.value=o.target.value)}}),r.addEventListener("change",o=>{let l=o.target.dataset.colorTokenVar;l!==void 0&&a(o.target,l,!0)})}function sf(r){if(!xi(r,"style"))return;let a=(o,l)=>{let h=String(o.value||"").trim();if(h){if((l==="--sve-heading-weight"||l==="--sve-body-weight")&&!Fd(l==="--sve-heading-weight"?"heading":"body",h)){let y=be(l);y&&(o.value=y);return}Re(l,h),(l==="--sve-heading-weight"||l==="--sve-body-weight")&&ir()}},s=o=>{let l=Qi(o);if(!l)return;Re(o,l);let h=k(`[data-color-token-var="${CSS.escape(o)}"], [data-style-var="${CSS.escape(o)}"]`,r),d=k(`[data-color-var="${CSS.escape(o)}"]`,r);if(h){let y=h.tagName==="SELECT"?Array.from(h.options).map(S=>S.value):[];(!y.length||y.includes(l))&&(h.value=l)}d&&(d.value=gi(l,d.value))};r.addEventListener("click",o=>{let l=o.target.closest("[data-reset-token]");if(l){s(l.dataset.resetToken);return}if(o.target.closest("#"+e+"-reset-style")){Vd();return}if(o.target.closest("#"+e+"-reset-all")){Ll();return}if(o.target.closest("#"+e+"-heading-font-apply")){rr("heading");return}o.target.closest("#"+e+"-body-font-apply")&&rr("body")}),r.addEventListener("change",o=>{let l=o.target.dataset.styleVar;l!==void 0&&a(o.target,l)}),r.addEventListener("input",o=>{if(o.target.tagName!=="SELECT")return;let l=o.target.dataset.styleVar;l!==void 0&&a(o.target,l)}),r.addEventListener("keydown",o=>{o.key==="Enter"&&(o.target.id===e+"-heading-font"?(o.preventDefault(),rr("heading")):o.target.id===e+"-body-font"&&(o.preventDefault(),rr("body")))})}function of(r){if(!xi(r,"audio"))return;let s=wl().path||"assets.audio",o=k("#"+e+"-audio-url",r),l=k("#"+e+"-audio-start-enabled",r),h=k("#"+e+"-audio-start-time",r);if(!o)return;let d=()=>{let S=o.value.trim(),w=D(u.config,s);if(typeof w=="string"&&w===S){Xl(r,S);return}Ue(u.config,s,S),Te("Audio diperbarui"),Xl(r,S)},y=()=>{if(!l||!h)return;let S=o.value.trim(),w=l.checked?ar(h.value):0,E=jd(S,w);o.value=E,h.disabled=!l.checked,l.checked&&(h.value=xn(w)),Ue(u.config,s,E),Te(w>0?"Waktu mulai audio diperbarui":"Waktu mulai audio dimatikan")};o.addEventListener("paste",()=>{setTimeout(d,0)}),o.addEventListener("change",d),l?.addEventListener("change",()=>{h&&(h.disabled=!l.checked,l.checked&&ar(h.value)<=0&&(h.value="0:00",h.focus()),y())}),h?.addEventListener("change",y)}function lf(r){xi(r,"compat")}function cf(){Object.values(u.editors).forEach(r=>{r&&zi(r,!0)})}function uf(r,a=""){let s=k("#"+e+"-body");if(!s||!ke()||(u.sourceDirty||!u.doc)&&!Ae()||(r=String(r||"").trim(),r&&!mt(r)))return!1;let o=r&&hn().find(w=>w.path===r),l=li(),h=w=>Zi(w).some(E=>E.path===r||(E.type==="repeater"||oe(E)==="repeater-image")&&r.startsWith(E.path+".")),d=r&&(l.find(w=>Z(w)===a&&h(w))||l.find(h))||l.find(w=>Z(w)===a);if(!o&&!d)return!1;u.search="";let y=k("#"+e+"-search");y&&(y.value=""),u.open||Dt(!0),Vt("content");let S;if(o){let w=k(`[data-section-card="${CSS.escape(Z(d))}"]`,s);if(!w)return!1;cn(w),S=k(`[data-image-path="${CSS.escape(r)}"]`,w),S||(S=k(".chev",w))}else{let w=k(`[data-section-card="${CSS.escape(Z(d))}"]`,s);if(!w)return!1;cn(w),S=r&&k(`[data-field-path="${CSS.escape(r)}"]`,w),S||(S=k(".chev",w))}return S?(S.focus({preventScroll:!0}),S.scrollIntoView({block:"nearest",behavior:"auto"}),!0):!1}let sc='#builder-canvas-boundary iframe[title="HTML Mode preview"][srcdoc]';function pf(r,a,s){if(typeof a!="string"||!a||a.length>256||typeof s!="string"||!s.startsWith("html-mode-preview:")||s.length>256)return null;let o=r?.getAttribute("srcdoc")||"";if(!o)return null;let l=u.canvasPickSources.get(r);if(!l||l.source!==o){let S=document.createElement("template");S.innerHTML=o,l={source:o,root:S.content.querySelector("#scalev-html-mode-preview-root"),scripts:C("script",S.content).map(w=>w.textContent).join(`
`)},u.canvasPickSources.set(r,l)}if(!l.root||!l.scripts.includes(JSON.stringify(s)))return null;let h=l.root.querySelector(`[data-scalev-inspector-id="${CSS.escape(a)}"]`);if(!h)return null;let d=h.matches("[data-sve-field]")?h:h.querySelector("[data-sve-field]")||h.closest("[data-sve-field]"),y=h.closest("[data-section-id], [data-sve-section]");return{path:d?.getAttribute("data-sve-field")||"",sectionHint:y?.getAttribute("data-section-id")||y?.id||y?.getAttribute("data-sve-section")||""}}function hf(){if(u.canvasPickMessageBound)return;u.canvasPickMessageBound=!0;let r=location.href,a=null;window.addEventListener("message",s=>{if(location.href!==r)return;let o=s.data;if(!o||o.type!=="scalev-html-mode-inspector-selected"||s.origin!=="null"||typeof o.inspectorId!="string"||!o.inspectorId||o.inspectorId.length>256||typeof o.previewId!="string"||o.previewId.length>256)return;let l=C(sc).find(G=>G.contentWindow===s.source);if(!l||!l.sandbox.contains("allow-scripts")||l.sandbox.contains("allow-same-origin"))return;let h=l.getAttribute("srcdoc"),d=location.href,{open:y,tab:S,sourceDirty:w}=u,E=u.performance.configCommitCount,T=V("html"),L=V("js");cancelAnimationFrame(a),a=requestAnimationFrame(()=>{if(a=null,location.href!==d||!l.isConnected||!l.matches(sc)||l.contentWindow!==s.source||l.getAttribute("srcdoc")!==h||u.open!==y||u.tab!==S||u.sourceDirty!==w||u.performance.configCommitCount!==E||V("html")!==T||V("js")!==L)return;let G=pf(l,o.inspectorId,o.previewId);G&&(G.path||G.sectionHint)&&uf(G.path,G.sectionHint)})})}function df(){let r="https://wa.me/"+p+"?text="+encodeURIComponent(f);window.open(r,"_blank","noopener,noreferrer")}function ff(){performance.mark("sve-styles-start"),mf(),performance.mark("sve-styles-critical-done"),Ft(gf,50)}function mf(){if(document.getElementById(e+"-style-critical"))return;let r=document.createElement("style");r.id=e+"-style-critical",r.textContent=`#${e},
#${e} * {
