// ==UserScript==
// @name         Scalev Multi Upload (Reseller)
// @namespace    nikahin-reseller
// @version      1.18.0
// @updateURL    https://raw.githubusercontent.com/hasyaapp/visual-editor/main/scripts/scalev-multi-upload-reseller.user.js
// @downloadURL  https://raw.githubusercontent.com/hasyaapp/visual-editor/main/scripts/scalev-multi-upload-reseller.user.js
// @description  Pilih banyak foto sekaligus dan cari foto lama di Media Library Scalev
// @match        https://app.scalev.com/pages/*
// @grant        none
// @run-at       document-idle
// ==/UserScript==

// Untuk reseller/kreator: cukup pilih banyak berkas, sisanya otomatis.
// Foto langsung masuk ke Media Library, dan foto lama bisa dicari lewat kotak cari.
...
(function () {
  ...
  // Keberhasilan dinilai dari PUT ke R2 (langkah nyata unggah bytes).
  // Jumlah kartu di daftar tidak bisa dipakai: Scalev me-refresh daftar,
  // bukan menambah kartu satu per satu.
  ...
  window.fetch = async function (input, init) {
    ...
    if (/r2\.cloudflarestorage\.com/.test(url) && method === 'PUT') putCount += 1;
    return response;
  };
  ...
  function say(text) {
    ...
    if (!host) return;
    let note = host.querySelector('[data-sve-say]');
    if (!note) {
  ...
  async function uploadOne(input, file) {
    ...
    while (Date.now() < deadline) {
      await sleep(POLL_MS);
      if (putCount > before) return true;
    }
    return false;
  }
  ...
  document.addEventListener('change', async event => {
    ...
    for (const [i, file] of files.entries()) {
      say((i + 1) + '/' + files.length);
      const ok = await uploadOne(input, file);
    ...
    say(failed.length
      ? (files.length - failed.length) + '/' + files.length + ' · gagal:\n' + failed.join('\n')
