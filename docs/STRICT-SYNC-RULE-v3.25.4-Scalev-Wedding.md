...
- dilarang raster image base64 (`data:image/png|jpeg|webp|gif|avif;base64`) pada
  HTML, CSS, JavaScript, Additional Head, maupun nilai `CONFIG`; gambar raster
  memakai URL `https://`. Icon `data:image/svg+xml` boleh, dan data URI di atas
  4 KB memicu warning.
...
- [ ] tidak ada raster image base64 (`data:image/...;base64`) di source maupun
    `CONFIG`;
