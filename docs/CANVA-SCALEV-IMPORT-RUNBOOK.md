...
> **Catatan:** field `template.type: "custom-page"` yang dipakai di fixture
> lama tidak lagi dibaca oleh editor. Editor hanya memakai `template.name`
> (untuk label UI). `custom-page` tetap menjadi terminologi internal
> runbook untuk menandai fixture yang bukan wedding template; editor tidak
> memvalidasi 21 section wedding bila `sections[]` kosong, dan
> `validateCompleteTemplateDocument` tidak cek `template.type`.
...
- custom-page tidak dipaksa menjadi 21-section wedding.
